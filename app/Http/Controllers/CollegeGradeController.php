...
class CollegeGradeController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            if(session('user')->role =="Super Admin"){
                ...
                $Subjects = SubjectTbl::select("subject_tbls.*",
                "schedules_tbls.id as sched_id","schedules_tbls.staff_id as staff_id")
    ...
    public function show(Request $request, $id)
    {
        ...
        if(count($sectionOne)>0){
            if ($request->ajax()) {
        $students = StudentsTbl::select(DB::raw('@rownum :=@rownum+1 as rowNum'),
        "students_tbls.id as std_id","students_tbls.last_name","enrollment_tbls.*","grades.*","grades.subject_id as grade_id",
        ...
        ->join("grades","students_tbls.id","grades.student_id")
    ...
    {
        //
        $data['getDataById'] = Grade::select("grades.*",
