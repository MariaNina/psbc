...
class StaffAttendanceController extends Controller
{
    ...
    public function index(Request $request)
    {
        if ($request->ajax()) {
            ...
            $staffs = StaffsTbl::all();  
            
            if($request->dept != 'all'){
                $staffs = $staffs->where('Department',$request->dept);
            }
            foreach($staffs as $staff){
                ...
                // $shift_hours = $time_setting->required_time;
                 ...
                 $AttendanceTbl = AttendanceSummaryView::where('bio_id',$staff->bio_id)
                ->whereBetween('attendance_summary_view.punch_date',[$begin,$end])->get();
    ...
    //                 $excel_time = trim($worksheet->getCellByColumnAndRow(2, $row)->getValue());
    //                 $excel_time = Date::excelToTimestamp($excel_time);
    //                 $punch_time = gmdate("H:i:s", $excel_time);
    //                 $punch_date = gmdate("Y-m-d", $excel_time);
    //                 $punch_day = gmdate("D", $excel_time);
    //                 $punch_type = trim($worksheet->getCellByColumnAndRow(4, $row)->getValue());
    ...
    //                 $staff->punch_time = $punch_time;
    //                 $staff->punch_type = $punch_type;
        ...
        if (!empty($_FILES["import_excel"]["tmp_name"])) {
            ...
            // foreach ($object->getWorksheetIterator() as $worksheet) {
                ...
                for ($row = 5; $row <= $highestRow; $row++) {
                    ...
                    if($excel_date != ""){
                        // $excel_date = Date::excelToTimestamp($excel_date);
                ...
                for ($row = 5; $row <= $highestRow; $row++) {
                    ...
                    $excel_time = trim($worksheet->getCellByColumnAndRow(6, $row)->getValue());
                    if($excel_time != ""){
                        // $excel_time = Date::excelToTimestamp($excel_time);
                        $punch_time = $excel_time;
                        $punch_type = 1;
