$(function(){
    ...
    const addModal = $('#gradStudCurriculumModal');
    const updateModal = $('#editGradCurriculumModal');
    const addForm = $('#addGradStudForm');
    const updateForm = $('#editGradStudForm');
    ...
    function createData() {
     ...
     $('body').on('click', '.editData_GraduateStudies', function(e) {
        // e.preventDefault();
        id = $(this).data('id');
        // console.log(id)
        $.ajax({
            ...
            success: function(result) {
                console.log(result)
                console.log(result.getDataById[0].curriculum_year)
                let pmHtml = ''; //programs majors html
                let syHtml = ''; //school year html
                let lhtml = ''; //levels html
                // display data to updateschoolyearform
                updateModal.modal('show');
                $('#upd_gradCurriculumYear').val(result.getDataById[0].curriculum_year);
                $('#upd_gradCurriculumDescription').val(result.getDataById[0].curriculum_description);
                ...
                result.programsMajorsGradStud.forEach(pm => {
                ...
                result.grad_levels.forEach(l => {
                    lhtml += '<option value="'+l.id+'" '+((level_id == l.id) ? "selected" : "")+'>'+l.level_name+'</option>';
                });
                $('#upd_gradProgramMajor').html(pmHtml);
                $('#upd_gradSchoolYear').html(syHtml);
                $('#upd_gradLevel').html(lhtml);
                // console.log(pmHtml)
            }
        });
    });
