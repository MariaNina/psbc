...
const getAllPayslipData=(e)=>{
    ...
    axios.get(page_url+'/all?date_id='+sub_id).then(resp =>{
        ...
        let canteen = null;
        let tuition_fee = null;
        let cash_advance = null;
        let sss = null;
        let others = null;
        let late_undertime = null;
        if(resp.data.deductions[i]!=null){
         canteen = resp.data.deductions[i].canteen;
         tuition_fee = resp.data.deductions[i].tuition_fee;
         cash_advance = resp.data.deductions[i].cash_advance;
         sss = resp.data.deductions[i].sss;
         others = resp.data.deductions[i].others;
        late_undertime = resp.data.deductions[i].late_undertime;
        }else{
            canteen = 0;
         tuition_fee = 0;
         cash_advance = 0;
        ...
        paysHtml+='<span class="font-weight-bold">Employee Payslip:'+Staff.bio_id+'</span><span id="payslip_id"></span><br>';
        paysHtml+='<span class="font-weight-bold">Pay Type:</span> Semi-Monthly<br>';
        paysHtml+='<span class="font-weight-bold">Name: '+Staff.last_name+', '+Staff.first_name+'</span><span id="full_name"></span><br><span class="font-weight-bold">Employee Id: '+Staff.bio_id+'</span><span id="staff_id"></span><br>';
        paysHtml+='<span class="font-weight-bold">Department/Position: '+Staff.Department+'/'+Staff.position+'</span><span id="dept_pos"></span><br></div>';
        ...
        paysHtml+='<td></td></tr><tr style="line-height: 0px;"><td><div class="row"><div class="col-6">No. of Days:</div><div class="col-6" id="no_days">'+required_day.weekdays+'</div></div></td><td>';
        ...
        paysHtml+='<tr style="line-height: 0px;"><td><div class="row"><div class="col-6">Special Allow.:</div><div class="col-6">'+parseInt(special_allowance).toFixed(2)+'</div></div></td><td><div class="row"><div class="col-3">SSS: </div><div class="col-3">'+sss+'</div><div class="col-3"></div>';
        ...
        paysHtml+='<div class="row"><div class="col-3">Deduction</div><div class="col-3"></div><div class="col-3"></div><div class="col-3" id="deduction">'+total_deduction+'</div></div></td>';
...
//add to buttons the functions
    ...
    $('#payslipFilter').submit(getPayslipDatas); //adding function to filter button
